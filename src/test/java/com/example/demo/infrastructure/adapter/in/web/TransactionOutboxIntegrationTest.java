...
class TransactionOutboxIntegrationTest {
    ...
    void setUp() {
        ...
        var userEntity = new com.example.demo.infrastructure.adapter.out.persistence.entity.UserJpaEntity(
                UUID.fromString(userId), "Integration", "Tester", null
        );
        userRepository.save(userEntity);

        // Save test account
        var accountEntity = new com.example.demo.infrastructure.adapter.out.persistence.entity.AccountJpaEntity(
    ...
    void shouldProcessDepositOutboxEventAsynchronously() throws Exception {
        ...
        List<OutboxEventJpaEntity> pendingOutbox = outboxRepository.findByStatusOrderByCreatedAtAsc(OutboxStatus.PENDING);
