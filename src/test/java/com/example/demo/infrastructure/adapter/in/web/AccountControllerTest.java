...
class AccountControllerTest {
	...
	void shouldReturn200WhenDepositIsSuccessful() throws Exception {
		...
		// Act & Assert
		mockMvc.perform(post("/api/accounts/deposit").header("X-User-Id", requesterId) // Added Security Header
	...
	void shouldReturn400BadRequestWhenDepositAmountIsNegative() throws Exception {
		...
		// Act & Assert
		mockMvc.perform(post("/api/accounts/deposit").header("X-User-Id", "USER-123")
	...
	void shouldReturn403ForbiddenWhenUserAttemptsToTouchAnotherUsersAccount() throws Exception {
		...
		// Act & Assert
		mockMvc.perform(post("/api/accounts/deposit").header("X-User-Id", "HACKER-ID")
	...
	void shouldReturn200AndUpdatedAccountOnSuccessfulWithdrawal() throws Exception {
		...
		// Act & Assert
		mockMvc.perform(post("/api/accounts/withdraw").header("X-User-Id", requesterId)
	...
	void shouldReturn400BadRequestWhenInsufficientFunds() throws Exception {
		...
		mockMvc.perform(post("/api/accounts/withdraw").header("X-User-Id", "USER-123")
